...
class SelfLearningRAGSystem:
    ...
    async def enhanced_query(
        self, question: str, user_context: Dict = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_adaptive_query(
        self, question: str, query_id: str, k: int, strategy: str, user_context: Dict
    ) -> Dict[str, Any]:
    ...
    async def _graph_focused_retrieval(self, question: str, k: int) -> Dict[str, Any]:
        ...
        with self.base_system.driver.session() as session:
            ...
            if (
                schema_result
                and schema_result["has_chunks"]
                and schema_result["has_keywords"]
            ):
                ...
                graph_results = session.run(
                    weighted_graph_query,
                    {"keywords": keywords, "weights": self.connection_weights, "k": k},
                ).data()
            else:
                weighted_graph_query = """
                MATCH (n)
                ...
                graph_results = session.run(
                    weighted_graph_query, {"search_term": search_term, "k": k}
                ).data()

        graph_contexts = [
            r.get("c.content", r.get("content", "")) for r in graph_results[:k]
        ]
        # Filter empty contexts
        graph_contexts = [ctx for ctx in graph_contexts if ctx]
    ...
    async def _hybrid_optimized_retrieval(
    ...
    async def _context_aware_retrieval(
        self, question: str, k: int, user_context: Dict
    ) -> Dict[str, Any]:
        ...
        vector_results = self.base_system.vector_store.similarity_search(
            context_enhanced_query, k=k
        )
        contexts = [d.page_content for d in vector_results]
    ...
    async def _classify_query_type(self, question: str) -> str:
        ...
        response = await self.base_system.entity_analyzer_llm.agenerate(
        ...
        return (
            query_type
            if query_type
            in ["factual", "analytical", "procedural", "exploratory", "comparative"]
            else "general"
        )
    ...
    async def _track_chunk_usage(self, contexts: List[str], query_id: str):
        ...
        for context in contexts:
            # Convert hash to string for consistent dict key type
            chunk_hash = str(hash(context))
    ...
    async def _update_query_clusters(self):
        ...
        try:
            ...
            if n_clusters > 1:
                ...
                cluster_labels = self.query_clusterer.fit_predict(
                ...
                for i, metrics in enumerate(list(self.query_history)[-100:]):
                    if metrics.user_rating:
                        cluster_performance[cluster_labels[i]].append(
                            metrics.user_rating
                        )
                ...
                for cluster_id, ratings in cluster_performance.items():
                    if len(ratings) >= 3:
                        ...
                        if avg_rating < 2.5:
                            print(
                                f"📊 Poor performing cluster {cluster_id} identified for optimization"
                            )
    ...
    async def _optimize_connection_weights(self):
        ...
        if not getattr(self.base_system, "driver", None):
            for chunk_hash, stats in self.chunk_performance.items():
                ...
                if ratings:
                    self.connection_weights[chunk_hash] = (
                        sum(ratings) / len(ratings) / 5.0
                    )
            return
    ...
    def record_user_feedback(
        self, query_id: str, rating: float, clicked_sources: List[str] = None
    ):
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        avg_response_time = (
            statistics.mean([m.response_time for m in self.query_history])
            if total_queries > 0
            else 0
        )
        ...
        avg_rating = (
            statistics.mean([m.user_rating for m in rated_queries])
            if rated_queries
            else 0
        )
    ...
    def _save_learning_state(self):
        ...
        try:
            ...
            with open(self.learning_data_path / "learning_state.pkl", "wb") as f:
                # Type ignore for BufferedWriter compatibility
                pickle.dump(state, f)
