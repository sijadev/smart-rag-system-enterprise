...
from .interfaces import RetrievalStrategy, QueryContext, ILLMService, IVectorStore, IGraphStore
...
class SmartRAGSystem:
    ...
    def initialize(self):
        ...
        # Create LLM service
        try:
            self.llm_service = LLMServiceFactory.create(self.config.llm_provider, {
        ...
        try:
            self.vector_store = DatabaseFactory.create_vector_store(self.config.vector_store_type, {
            ...
            logger.warning(f"Vector store '{self.config.vector_store_type}' creation failed: {e}. Attempting direct mock import as fallback.")

            # Try registry fallback first
            try:
                self.vector_store = DatabaseFactory.create_vector_store('mock', {'collection_name': self.config.collection_name})
                logger.info("Using fallback mock vector store from registry")
                self.container.register_instance(IVectorStore, self.vector_store)
            except Exception:
                # If registry does not contain 'mock', import mock class directly
                try:
                    from src.adapters.mock_vector_store import MockVectorStore
                    self.vector_store = MockVectorStore({'collection_name': self.config.collection_name})
        ...
        if self.config.enable_graph_store:
            try:
                ...
                self.graph_store = DatabaseFactory.create_graph_store('neo4j', {
                ...
                if logger:
                    logger.warning(f"Neo4j store creation failed: {e}. Falling back to 'mock'.")
    ...
    async def query(self, query_text: str, strategy: Optional[RetrievalStrategy] = None, k: int = 5):
        ...
        try:
            if chosen == RetrievalStrategy.VECTOR_ONLY:
                strat = RetrievalStrategyFactory.create(RetrievalStrategy.VECTOR_ONLY, self.vector_store)
            elif chosen == RetrievalStrategy.GRAPH_ONLY:
                strat = RetrievalStrategyFactory.create(RetrievalStrategy.GRAPH_ONLY, self.graph_store)
            elif chosen == RetrievalStrategy.HYBRID:
                strat = RetrievalStrategyFactory.create(RetrievalStrategy.HYBRID, self.vector_store, self.graph_store, self.llm_service)
            elif chosen == RetrievalStrategy.SEMANTIC_SEARCH:
                strat = RetrievalStrategyFactory.create(RetrievalStrategy.SEMANTIC_SEARCH, self.vector_store, self.llm_service)
            else:
